...
import { EasyGSClient } from "@/lib/easygs-client";

/**
 * Minimal fake WebSocket implementing the subset EasyGSClient touches.
...
describe("EasyGSClient", () => {
  it("routes events to the matching chat handler", () => {
    const client = new EasyGSClient({
      url: "ws://test",
      reconnect: false,
  ...
  it("resolves newChat() via the server-assigned chat_id", async () => {
    ...
    expect(frame).toMatchObject({ type: "new_chat" });
  ...
  it("does not resolve newChat() from an older attach acknowledgement", async () => {
    const client = new EasyGSClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    client.connect();
    lastSocket().fakeOpen();
    client.onChat("existing-chat", () => {});
    const promise = client.newChat(1_000);
    const frame = JSON.parse(lastSocket().sent.at(-1) as string);

    lastSocket().fakeMessage({ event: "attached", chat_id: "existing-chat" });
  ...
  it("sends text-only message envelopes", () => {
    const client = new EasyGSClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
