...
describe("useSessions", () => {
  ...
  it("hydrates historical text messages", async () => {
    vi.mocked(api.fetchSessionMessages).mockResolvedValue({
      key: "websocket:chat-history",
      ...
      messages: [
        ...
        {
          role: "assistant",
          content: "start with a small run",
    ...
    expect(first.content).toBe("what should I try first?");
    expect(second.role).toBe("assistant");
    expect(second.content).toBe("start with a small run");
    expect(third.role).toBe("user");
    expect(third.content).toBe("follow-up");
  });
