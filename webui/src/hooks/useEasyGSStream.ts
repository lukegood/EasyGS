...
import type { StreamError } from "@/lib/easygs-client";
import type {
  InboundEvent,
  UIMessage,
} from "@/lib/types";
 ...
 * live events.
 */
export function useEasyGSStream(
  chatId: string | null,
  ...
  useEffect(() => {
    ...
    const handle = (ev: InboundEvent) => {
      if (
        ev.event === "stream_end" ||
        ev.event === "error" ||
        (ev.event === "message" && ev.turn_complete)
      ) {
        setIsAwaitingResponse(false);
      }
      ...
      if (ev.event === "message") {
        // Tool-call breadcrumbs collapse together into one compact trace group.
        ...
        // Runtime progress notices are not tool activity. Keep them separate so
        // the UI can render them as status messages instead of "Using a tool".
        if (ev.kind === "progress") {
          setMessages((prev) => [
            ...prev,
            {
