...
export interface EasyGSClientOptions {
  url: string;
  reconnect?: boolean;
...
export class EasyGSClient {
  ...
  constructor(private options: EasyGSClientOptions) {
  ...
  newChat(timeoutMs: number = 5_000): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      this.queueSend({ type: "new_chat", request_id: requestId });
    });
  }
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    if (parsed.event === "attached") {
      ...
      this.knownChats.add(parsed.chat_id);
      const isPendingNewChat =
        this.pendingNewChat &&
        (parsed.request_id === this.pendingNewChat.requestId ||
          (!parsed.request_id && !wasKnown));
      if (isPendingNewChat && this.pendingNewChat) {
  ...
  private createRequestId(): string {
    ...
    return `new-chat-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  }
