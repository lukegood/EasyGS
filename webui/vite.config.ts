...
export default defineConfig(({ mode }) => {
  ...
  const target = env.EASYGS_API_URL ?? "http://127.0.0.1:25685";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../easygs/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    },
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the EasyGS ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the EasyGS gateway;
