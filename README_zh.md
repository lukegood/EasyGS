...
<table>
  <tr>
    <td align="center" valign="top" width="55%">
      ...
      <img src="pics/EasyGS_UI.png" alt="EasyGS 本地 WebUI" height="260">
    </td>
...
大多数用户推荐使用下方“使用 Docker”一节中的一键安装。需要自行管理 Python、conda 和分析依赖的开发者或用户，仍可按照本节进行本地安装。
...
如果环境已经存在，可以跳过对应命令。`EasyGS_5` 提供 `fastq_to_vcf_analysis` 使用的 fastp、BWA、samtools、Picard、GATK、bcftools、bgzip 和 tabix 依赖。
...
推荐使用 Docker 运行 EasyGS。镜像同时包含 EasyGS 和五套分析环境，用户只需挂载持久化的 EasyGS home 目录和数据目录。

基础要求：

- Linux x86_64、Bash 和 curl
- Git、Docker Engine 和 Docker Compose
- 可用的 LLM provider 和 API key

### 一键安装

仓库根目录的一键安装脚本会检查宿主机依赖、克隆或更新 EasyGS、交互配置模型服务、构建镜像并启动服务：

```bash
curl -fsSL https://raw.githubusercontent.com/lukegood/EasyGS/master/install.sh | bash
```

默认安装目录为 `~/easygs`。如需指定安装目录、数据目录或 Git 分支：
...
安装配置保存在 `~/easygs/.env`。重复运行脚本会更新源码和容器，同时保留模型、API key、飞书和邮件配置。脚本不会静默安装缺失的宿主机依赖；Mamba、R 和生信分析工具均保留在镜像内部。
...
# 设置 EASYGS_HOME_DIR、EASYGS_DATA_DIR、模型和 provider 凭证。
...
| 基因挖掘与功能解析 | GWAS分析 (`gwas_analysis`) | 使用 rMVP 方法进行全基因组关联分析。 |
...
## :violin: Research Mode 与分析工作流

EasyGS 区分普通聊天和分析型使用。科学分析工作流默认启用 Research Mode，避免多次分析结果相互干扰。

典型分析流程如下：

1. 用户提供需求、文件路径和关键参数。
2. EasyGS 检查必需文件、托管资源、环境和工具。
3. 长时间分析作为后台 agentic workflow 提交。
4. 每次实际工具调用记录为一个 action，输出写入目标目录。
5. EasyGS 生成结果摘要，用户之后仍可查询工作流状态和结果。
...
部分工作流需要大型参考文件，这些文件不会打包进 EasyGS。默认资源目录为：
...
蛋白功能注释仍仅支持玉米。PFAM 富集支持玉米、小麦和水稻；小麦和水稻的多 GB 注释文件采用流式处理。
...
工具根据 `species=maize/wheat/rice` 选择对应 GFF3，并在运行前检查 BED 与 GFF3 的染色体命名是否兼容。
...
工具自动选择物种矩阵，并对矩阵第一列进行完整基因 ID 精确匹配。
...
富集工具只要求基因列表和物种，自动选择对应映射及注释数据库。资源缺失时，EasyGS 会在执行前报告准确的预期路径。
...
## :headphones: 扩展 EasyGS

EasyGS 通过 tool、agentic workflow 和 skill 组合扩展能力。新增分析功能通常包括：

1. 在 `easygs/agent/tools` 下增加工具实现。
2. 实现 `prepare_run()` 和 `to_metadata()`，支持 workflow action 执行。
3. 在 `easygs/agent/workflows.py` 中注册 workflow action。
4. Agent 需要领域指导时，在 `easygs/skills` 下增加 skill。

项目规范和验证流程见 [新增 Skill 与 Tool](docs/adding_skill_and_tool_zh.md)。
