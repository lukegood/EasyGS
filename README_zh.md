...
EasyGS 的分析工具依赖多个 conda 环境。请同时下载发布包提供的 `env_all/` 环境文件目录，或使用源码中的 `env_all/` 目录，然后创建环境：
...
容器内请使用 `/data/...` 路径引用挂载的数据文件。更多说明见 [container/README.md](container/README.md)。
