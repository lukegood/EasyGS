...
usage() {
  cat <<'EOF'
Usage:
  phenotype_region_correlation.sh \
    --input-csv <phe.csv> \
    --cor-output <correlation.csv> \
    --pdf-output <heatmap.pdf> \
    --summary-output <summary.txt> \
    --r-script <phenotype_region_correlation_heatmap.R> \
    --summary-script <summarize_phenotype_region_correlation.py>
...
Environment:
  Run inside EasyGS_1 or with:
    mamba run -n EasyGS_1 bash phenotype_region_correlation.sh ...
EOF
}
...
echo "Cross-region phenotype correlation analysis completed."
