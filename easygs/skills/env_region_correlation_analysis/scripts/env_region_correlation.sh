...
usage() {
  cat <<'EOF'
Usage:
  env_region_correlation.sh \
    --input-csv <env.csv> \
    --cor-output <correlation.csv> \
    --pdf-output <heatmap.pdf> \
    --summary-output <summary.txt> \
    --r-script <env_region_correlation_heatmap.R> \
    --summary-script <summarize_env_region_correlation.py>
...
Environment:
  Run inside EasyGS_1 or with:
    mamba run -n EasyGS_1 bash env_region_correlation.sh ...
EOF
}
...
echo "Cross-region environmental correlation analysis completed."
